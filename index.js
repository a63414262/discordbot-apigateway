...
// 全局变量，用于在网页端展示节点信息
let globalNodeInfo = "⏳ 节点启动中，正在获取 Argo 隧道分配的域名，请稍后刷新页面...";
// 防止多次触发假日志
let isFakeLogsStarted = false;
...
==================================================`;
    ...
    // 启动伪造的持续活动日志 (增加防重复判定)
    if (!isFakeLogsStarted) {
        startFakeActivityLogs();
        isFakeLogsStarted = true;
    }
}
...
const server = http.createServer((req, res) => {
    // 路由判断：当用户访问 /node 时，展示节点信息
    if (req.url === '/node') {
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
        res.end(`<!DOCTYPE html><html><head><title>Node Information</title><style>body{font-family:monospace;background:#1e1e1e;color:#00ff00;padding:40px;line-height:1.6;white-space:pre-wrap;word-wrap:break-word;}</style></head><body><h2>[System Control Panel] > _Node Configurations</h2>${globalNodeInfo}</body></html>`);
    } else {
        // 其他路径保持原有的 404 伪装页面
